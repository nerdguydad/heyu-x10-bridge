...
void flag_intr();
void cleanup_files();
void reopen_tty_handler(int signo);
...
/*
 * Signal handler for SIGUSR1: close and reopen the CM11A serial port.
 * This is the equivalent of MisterHouse's "un-stick a stuck CM11" operation —
 * it cycles only the serial port without restarting the relay, engine, or
 * aux daemons.  RF reception from the W800RF32A continues uninterrupted.
 */
void reopen_tty_handler(int signo)
{
   extern int setup_tty();
   extern int tty;

   syslog(LOG_NOTICE, "relay: reopening CM11A serial port (SIGUSR1)\n");

   /* Close the stuck serial port */
   if ( tty >= 0 )
      close(tty);

   /* Reopen without locking — we already hold the lock */
   setup_tty(0);

   /* Reset DTR/RTS to initial state */
   port_status_init(tty, DTR_INIT, RTS_INIT);

   /* Re-install this signal handler (some systems reset it) */
   signal(SIGUSR1, reopen_tty_handler);
}
...
int start_relay ( char *tty_name )
{
   ...
   /* is a relay in place ? */
    ...
    (void) signal(SIGUSR1, reopen_tty_handler);
